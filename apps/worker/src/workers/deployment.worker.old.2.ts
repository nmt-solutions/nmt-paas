...
const deploy = async (userId: string, deploymentId: number) => {
  ...
  try {
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      pushStream.on("data", async (chunk: Buffer) => {
          ...
          try {
            ...
            if (data.status === "Pushing" && data.progressDetail?.total) {
              ...
              if (roundedPercent > lastPercent) {
                ...
                await createDeploymentLogs({
                  deploymentId,
                  message: `[GHCR] Pushing ${roundedPercent}%`,
                  createdBy: userId,
                });
              }
    ...
    docker.modem.demuxStream(deploymentStream, stdout, stderr);

    const handleLogStream = (stream: PassThrough) => {
    ...
    handleLogStream(stdout);
    handleLogStream(stderr);
